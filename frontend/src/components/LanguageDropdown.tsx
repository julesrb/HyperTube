            ...
            {isOpen && <div className="absolute top-10 right-10 z-50 p-8">
                <div className="flex flex-col gap-1 items-start bg-white py-4 px-5 custom-shadow-s border">
                    {languages.map((language, i) => (
                        <button key={language} className={"custom-underline text-xl font-hairline" + (selectedLanguage[i] ? " custom-selected" : "")} onClick={() => handleSwitchLanguage(i)}>{language}</button>
                    ))}
                </div>
            </div>}
        </div>
