...
func (h *Handler) LoginFortyTwo(w http.ResponseWriter, r *http.Request) {
	if h.fortyTwo == nil {
		respond.Error(w, http.StatusServiceUnavailable, "OAUTH_NOT_CONFIGURED", "42 OAuth is not configured")
		return
	}
	...
	authURL, err := h.fortyTwo.AuthCodeURL(state)
	if err != nil {
		if errors.Is(err, ErrOAuthNotConfigured) {
			respond.Error(w, http.StatusServiceUnavailable, "OAUTH_NOT_CONFIGURED", "42 OAuth is not configured")
			return
		}
		respond.Error(w, http.StatusInternalServerError, "INTERNAL_ERROR", "failed to start 42 OAuth")
		return
	}

	http.SetCookie(w, oauthStateCookie(r, state, int(oauthStateTTL.Seconds())))
	http.Redirect(w, r, authURL, http.StatusFound)
}

func (h *Handler) CallbackFortyTwo(w http.ResponseWriter, r *http.Request) {
	if h.fortyTwo == nil {
		h.redirectOAuthError(w, r, http.StatusServiceUnavailable, "OAUTH_NOT_CONFIGURED", "42 OAuth is not configured")
		return
	}
	...
	if providerError := strings.TrimSpace(r.URL.Query().Get("error")); providerError != "" {
		...
		if state == "" || !validOAuthState(r, state) {
	...
	state := strings.TrimSpace(r.URL.Query().Get("state"))
	if code == "" || state == "" || !validOAuthState(r, state) {
		h.clearOAuthState(w, r)
		h.redirectOAuthError(w, r, http.StatusBadRequest, "INVALID_OAUTH_STATE", "invalid OAuth state")
		return
	}
	...
	identity, err := h.fortyTwo.Exchange(r.Context(), code)
	if err != nil {
		h.redirectOAuthError(w, r, http.StatusBadGateway, "OAUTH_EXCHANGE_FAILED", "failed to exchange 42 authorization code")
		return
	}
...
func validOAuthState(r *http.Request, state string) bool {
