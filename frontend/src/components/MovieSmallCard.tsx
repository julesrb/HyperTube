...
export default function MoviesSmallCard({movie} : {movie: tmovies}) {
    return (
        <div className="relative w-48 h-72 overflow-hidden cursor-pointer group custom-border rounded-sm">
            <Image className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105" width={100} height={100} src={"/images/" + movie.src} alt={"poster of film: " + movie.title}/>
